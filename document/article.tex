...
where $\sigma_i(X)$ denotes the $i$th largest singular value of $X$ and $\|X\|_*$ is the called the nuclear norm. The main point of this relaxation is that the nuclear norm is a convex function and thus can be optimized efficiently via semidefinite programming or by iterative soft thresholding algorithms.
...
\begin{equation}
    \begin{array}{ll}
    \mbox{minimize}_{X,Y}   & \frac{1}{2} \|P_{\Omega}(XY) - P_{\Omega}(M)\|^2_F \\
    \end{array}
